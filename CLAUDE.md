# CLAUDE.md: Rishik Rontala's entry for Acodemic X G.I.R.L.S. Global SDG Hackathon
...
**Deadline: Sun Sep 27, 2026 · 12:45 AM EDT.** Event page: https://acodemic-hackathon.devpost.com/
...
- **GitHub Pages deploys automatically from whatever branch you push** (`.github/workflows/pages.yml` builds `dist/`, `build/`, `out/` or `site/`, or a root `index.html`). Live URL: **https://rishikrrontala-bot.github.io/acodemic-hackathon/**. After each deploy, check the Actions run succeeded and load the URL headlessly.
- Finish on `main` (judges read the default branch). Try `git push origin HEAD:main`. If that's refused, open a PR and merge it with the REST API (`gh api -X POST repos/rishikrrontala-bot/acodemic-hackathon/pulls -f title=… -f head=<branch> -f base=main`, then `gh api -X PUT repos/rishikrrontala-bot/acodemic-hackathon/pulls/<n>/merge -f merge_method=squash`). If that's refused too, make "merge the PR" step 1 of HANDOFF.md.
